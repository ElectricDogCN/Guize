...
class TestPocProgram(unittest.TestCase):
    ...
    def temp_repo(self):
        ...
        shutil.copy2(self.repo / "specs" / "coordination" / "program-plan.yaml", coordination / "program-plan.yaml")
    ...
    def make_terminal(self, root, task_id="POC-003", status="pass"):
        ...
        for sample_id in plan["sampleIds"]:
            ...
            samples.append({"id": sample_id, "immutableId": f"fixture-{sample_id.lower()}", "checksum": "sha256:" + "a" * 64, "approved": True, "approvalRef": approval_ref})
        environment = {key: ({"fixed": "value"} if key == "inference_parameters" else f"captured-{key}") for key in plan["environment"]["captureBeforeExecution"]}
        measurements = []
        ...
        if plan["pocId"] == "POC-02":
            provenance = {"input_sha256": "sha256:" + "b" * 64, "output_sha256": "sha256:" + "c" * 64, "encoder_parameters": {"codec": "av1", "crf": 30, "preset": "medium"}}
        execution_ref = f"{plan['evidencePath']}/execution.yaml"
        execution = {"schemaVersion": 1, "pocId": plan["pocId"], "taskId": task_id, "evidencePath": plan["evidencePath"], "executor": implementer, "environmentCaptured": environment, "commands": ["bounded-test-command --fixture approved"], "rawOutputRefs": [raw_ref], "samples": samples, "measurements": measurements, "provenance": provenance, "notes": None}
        write_yaml(root / execution_ref, execution)
        result_ref = f"{plan['evidencePath']}/result.yaml"
        result = {"schemaVersion": 1, "pocId": plan["pocId"], "taskId": task_id, "status": status, "evidencePath": plan["evidencePath"], "executionRef": execution_ref, "decision": status, "rationale": "fixture result backed by existing raw evidence and complete measurements", "reviewer": reviewer, "approvedAt": "2026-09-03T00:00:00Z", "approval": "approved"}
        ...
        entry.update({"status": status, "resultRef": result_ref, "decision": status, "reviewer": reviewer, "approvedAt": "2026-09-03T00:00:00Z"})
    ...
    def test_13_plan_status_is_immutable(self):
        def mutate(root):
            path = self.plan_path(root, "POC-003"); data = load_yaml(path); data["status"] = "running"; data["resultStatus"] = "running"; write_yaml(path, data)
        self.assert_invalid(mutate, "canonical plan")

    def test_14_prefilled_plan_command_rejected(self):
        def mutate(root):
            path = self.plan_path(root, "POC-005"); data = load_yaml(path); data["protocol"]["commands"] = ["x"]; write_yaml(path, data)
        self.assert_invalid(mutate, "canonical plan")

    def test_15_prefilled_plan_measurement_rejected(self):
        def mutate(root):
            path = self.plan_path(root, "POC-009"); data = load_yaml(path); data["protocol"]["measurements"][0]["actual"] = 1; write_yaml(path, data)
        self.assert_invalid(mutate, "canonical plan")
    ...
    def test_17_prefilled_plan_reviewer_rejected(self):
        def mutate(root):
            path = self.plan_path(root, "POC-008"); data = load_yaml(path); data["review"]["reviewer"] = "x"; write_yaml(path, data)
        self.assert_invalid(mutate, "canonical plan")

    def test_18_catalogue_sample_must_remain_pending(self):
        def mutate(root):
            path = root / "specs" / "poc" / "samples.yaml"; data = load_yaml(path); data["samples"][0]["approvalState"] = "approved"; write_yaml(path, data)
        self.assert_invalid(mutate, "approvalState must remain pending")

    def test_19_catalogue_sample_identity_must_remain_tbd(self):
        def mutate(root):
            path = root / "specs" / "poc" / "samples.yaml"; data = load_yaml(path); data["samples"][0]["immutableId"] = "fixture-sample"; write_yaml(path, data)
        self.assert_invalid(mutate, "immutableId must remain TBD")

    def test_20_required_measurement_cannot_be_removed(self):
        def mutate(root):
            path = self.plan_path(root, "POC-001"); data = load_yaml(path); data["protocol"]["measurements"] = [item for item in data["protocol"]["measurements"] if item["id"] != "vm_reboot_recovery"]; write_yaml(path, data)
    ...
    def test_21_poc002_encode_provenance_gate_cannot_be_removed(self):
        def mutate(root):
            path = self.plan_path(root, "POC-002"); data = load_yaml(path); data["protocol"]["measurements"] = [item for item in data["protocol"]["measurements"] if item["id"] != "encode_provenance_complete"]; write_yaml(path, data)
        self.assert_invalid(mutate, "missing frozen required measurements")

    def test_22_poc010_secret_roundtrip_gate_cannot_be_removed(self):
        def mutate(root):
            path = self.plan_path(root, "POC-010"); data = load_yaml(path); data["protocol"]["measurements"] = [item for item in data["protocol"]["measurements"] if item["id"] != "secret_value_roundtrip"]; write_yaml(path, data)
        self.assert_invalid(mutate, "missing frozen required measurements")
    ...
    def test_24_downstream_task_must_own_evidence_path(self):
        def mutate(root):
            path = self.pp_path(root); data = load_yaml(path); task = next(item for item in data["tasks"] if item["taskId"] == "POC-003"); task["outputPaths"] = [item for item in task["outputPaths"] if not item.startswith("evidence/POC-003")]; write_yaml(path, data)
        self.assert_invalid(mutate, "must own its Evidence path")

    def test_25_downstream_task_must_share_results_index(self):
        def mutate(root):
            path = self.pp_path(root); data = load_yaml(path); task = next(item for item in data["tasks"] if item["taskId"] == "POC-003"); task["sharedPaths"] = [item for item in task.get("sharedPaths", []) if item != "specs/poc/results-index.yaml"]; write_yaml(path, data)
    ...
    def test_27_terminal_result_ref_must_exist(self):
        def mutate(root):
            _, _, result = self.make_terminal(root); result.unlink()
    ...
    def test_28_terminal_execution_ref_must_exist(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); execution.unlink()
        self.assert_invalid(mutate, "executionRef must point to an existing file")

    def test_29_raw_evidence_must_exist(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); (root / data["rawOutputRefs"][0]).unlink()
        self.assert_invalid(mutate, "raw evidence file must exist")

    def test_30_approval_ref_must_exist(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); (root / data["samples"][0]["approvalRef"]).unlink()
        self.assert_invalid(mutate, "approvalRef must exist")

    def test_31_bad_checksum_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["samples"][0]["checksum"] = "x"; write_yaml(execution, data)
        self.assert_invalid(mutate, "checksum must be sha256")

    def test_32_placeholder_immutable_id_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["samples"][0]["immutableId"] = "TBD"; write_yaml(execution, data)
        self.assert_invalid(mutate, "immutableId")

    def test_33_empty_environment_value_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); key = next(iter(data["environmentCaptured"])); data["environmentCaptured"][key] = ""; write_yaml(execution, data)
        self.assert_invalid(mutate, "empty/placeholder environment values")

    def test_34_placeholder_environment_value_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); key = next(iter(data["environmentCaptured"])); data["environmentCaptured"][key] = "TBD"; write_yaml(execution, data)
        self.assert_invalid(mutate, "empty/placeholder environment values")

    def test_35_empty_string_measurement_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["measurements"][0]["actual"] = ""; write_yaml(execution, data)
        self.assert_invalid(mutate, "invalid/placeholder measurement values")

    def test_36_nonfinite_measurement_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["measurements"][0]["actual"] = float("inf"); write_yaml(execution, data)
        self.assert_invalid(mutate, "invalid/placeholder measurement values")

    def test_37_pass_false_boolean_gate_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); target = next(item for item in data["measurements"] if isinstance(item["actual"], bool)); target["actual"] = False; write_yaml(execution, data)
        self.assert_invalid(mutate, "PASS has false/non-true boolean gates")

    def test_38_missing_measurement_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["measurements"].pop(); write_yaml(execution, data)
        self.assert_invalid(mutate, "measurement IDs must exactly match plan")

    def test_39_poc002_missing_input_hash_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root, "POC-002"); data = load_yaml(execution); data["provenance"].pop("input_sha256"); write_yaml(execution, data)
        self.assert_invalid(mutate, "missing required provenance fields")

    def test_40_poc002_invalid_output_hash_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root, "POC-002"); data = load_yaml(execution); data["provenance"]["output_sha256"] = "sha256:x"; write_yaml(execution, data)
        self.assert_invalid(mutate, "provenance output_sha256")

    def test_41_poc002_empty_encoder_parameters_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root, "POC-002"); data = load_yaml(execution); data["provenance"]["encoder_parameters"] = {}; write_yaml(execution, data)
        self.assert_invalid(mutate, "encoder_parameters")

    def test_42_wrong_executor_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["executor"] = "another-implementer"; write_yaml(execution, data)
        self.assert_invalid(mutate, "executor")

    def test_43_self_review_rejected(self):
        def mutate(root):
            _, execution, result = self.make_terminal(root); executor = load_yaml(execution)["executor"]; result_data = load_yaml(result); result_data["reviewer"] = executor; write_yaml(result, result_data); index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["reviewer"] = executor; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "reviewer")

    def test_44_result_index_metadata_mismatch_rejected(self):
        def mutate(root):
            self.make_terminal(root); index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["approvedAt"] = "2026-09-04T00:00:00Z"; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "approvedAt")

    def test_45_nonterminal_fields_must_be_null(self):
        def mutate(root):
            index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["status"] = "blocked"; entry["decision"] = "pass"; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "nonterminal blocked decision must be null")

    def test_46_compound_secret_key_rejected(self):
        def mutate(root):
            path = root / "specs" / "poc" / "resources.yaml"; data = load_yaml(path); data["resources"][0]["productionApiKeyValue"] = "opaque"; write_yaml(path, data)
        self.assert_invalid(mutate, "secret-like key")

    def test_47_compound_auth_token_key_rejected(self):
        def mutate(root):
            path = root / "specs" / "poc" / "resources.yaml"; data = load_yaml(path); data["resources"][0]["authTokenValue"] = "opaque"; write_yaml(path, data)
        self.assert_invalid(mutate, "secret-like key")

    def test_48_secret_value_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["commands"] = ["tool --token=super-secret-value"]; write_yaml(execution, data)
        self.assert_invalid(mutate, "secret-like value")

    def test_49_result_path_escape_rejected(self):
        def mutate(root):
            self.make_terminal(root); index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["resultRef"] = "evidence/POC-003/../POC-999/result.yaml"; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "terminal resultRef")

    def test_50_raw_path_escape_rejected(self):
        def mutate(root):
            _, execution, _ = self.make_terminal(root); data = load_yaml(execution); data["rawOutputRefs"] = ["evidence/POC-003/../POC-999/raw.log"]; write_yaml(execution, data)
        self.assert_invalid(mutate, "raw evidence file must exist")
    ...
    def test_51_symlink_escape_rejected(self):
        ...
        try:
            ...
            data = load_yaml(execution); data["rawOutputRefs"] = ["evidence/POC-003/raw/link.log"]; write_yaml(execution, data); errors = CHECK.validate_repository(root)
            self.assertTrue(any("raw evidence file must exist" in error for error in errors), "\n".join(errors))
        finally:
            temp.cleanup()
    ...
    def test_53_program_manifest_requires_record_contracts(self):
        def mutate(root):
            path = root / "specs" / "poc" / "program.yaml"; data = load_yaml(path); data["executionRecordSchema"] = "wrong.yaml"; write_yaml(path, data)
        self.assert_invalid(mutate, "executionRecordSchema")

    def test_54_policy_requires_immutable_plans(self):
        def mutate(root):
            path = root / "specs" / "poc" / "policy.yaml"; data = load_yaml(path); data["canonicalPlansImmutable"] = False; write_yaml(path, data)
        self.assert_invalid(mutate, "canonicalPlansImmutable")

    def test_55_missing_concrete_task_spec_rejected(self):
        def mutate(root):
            self.make_terminal(root); self.task_path(root, "POC-003").unlink()
        self.assert_invalid(mutate, "concrete Task Spec is required")

    def test_56_generic_role_labels_rejected(self):
        def mutate(root):
            _, execution, result = self.make_terminal(root); self.write_task_spec(root, "POC-003", implementer="task-owner-agent", reviewer="independent-review-agent"); execution_data = load_yaml(execution); execution_data["executor"] = "task-owner-agent"; write_yaml(execution, execution_data); result_data = load_yaml(result); result_data["reviewer"] = "independent-review-agent"; write_yaml(result, result_data); index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["reviewer"] = "independent-review-agent"; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "must not reuse generic Program role labels")

    def test_57_task_spec_self_review_rejected(self):
        def mutate(root):
            _, execution, result = self.make_terminal(root); executor = load_yaml(execution)["executor"]; self.write_task_spec(root, "POC-003", implementer=executor, reviewer=executor); result_data = load_yaml(result); result_data["reviewer"] = executor; write_yaml(result, result_data); index = load_yaml(self.index_path(root)); entry = next(item for item in index["entries"] if item["taskId"] == "POC-003"); entry["reviewer"] = executor; write_yaml(self.index_path(root), index)
        self.assert_invalid(mutate, "implementer and reviewer must be distinct")

    def test_58_active_work_role_drift_rejected(self):
        def mutate(root):
            self.make_terminal(root); path = root / "specs" / "coordination" / "active-work.yaml"; active = load_yaml(path) if path.is_file() else {"version": 1, "policy": {}, "tasks": []}; active.setdefault("tasks", []).append({"taskId": "POC-003", "implementer": "drifted-implementer", "reviewer": "poc-003-reviewer"}); write_yaml(path, active)
