...
type: docs
status: approved
baseBranch: chore/GZ-001-repository-baseline
workBranch: docs/GZ-002-eprohub-style-restructure
...
本任务仅重构文档信息架构，不改变已经冻结的产品范围、技术决策和机器契约。

## 目标

1. 新建一份面向研发开工的统一设计总基线；
...
1. 检查新总文档内部链接和 Mermaid 语法可读性；
2. 检查 README 与 MANIFEST 的入口一致；
3. 检查 GZ-002 变更仅落在允许范围；
4. 执行仓库已有文档/治理检查；
5. Evidence 记录真实 GitHub 变更、已执行检查和未验证项。

## 风险

- 当前仓库尚无业务实现，因此类、接口、数据库表均只能作为研发规划基线；
- 旧文档中存在 POC 前假设，重构不得把假设升级为事实；
- 一份总文档过长时必须依赖清晰索引和折叠式阅读结构，而不是再次拆成大量碎片。

## 回滚

PR 合并前直接关闭 GZ-002 PR 并删除任务分支即可；若已合并，使用 Revert PR 恢复 README/MANIFEST 并删除新增总文档，不改写旧专题文档。