...
- live GitHub API verification of Ruleset include/exclude, Required Check, approval, CODEOWNERS, thread resolution, stale-review dismissal, latest-target-branch testing, deletion protection and non-fast-forward protection;
...
8. The mandatory pre-approval manual review of that exact HEAD found one new OPS-001 verifier gap: a Ruleset with Required Checks but `strict_required_status_checks_policy=false` could be accepted even though PRs were not required to test against the latest target branch. The verifier now rejects that state and has positive/negative regression tests.

No accepted rule was removed or converted to advisory-only behavior.

## Current validation state

The latest code and Evidence commits are newer than Gate #196. Their GitHub Governance Gate and fresh Codex review are authoritative; no prior Gate or review is reused as approval for the changed HEAD.
...
- OPS-001 #20 remains open. GitHub Branch Protection/Ruleset is not considered enabled until live API verification succeeds, including `strict_required_status_checks_policy=true`.
