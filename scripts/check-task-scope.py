...
def parse_args():
    ...
    parser.add_argument("--base", required=True, help="Base branch or commit to diff against, e.g. main")
...
def _looks_like_path_pattern(value):
    ...
    return (
        ...
        or bool(re.fullmatch(r"[A-Za-z0-9_.-]+", value))
    )
...
def match_pattern(filepath, pattern):
    ...
    if not os.path.splitext(pattern)[1] and "/" in pattern:
...
def _repo_subpath_prefix(repo_root):
    try:
        result = subprocess.run(["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, cwd=repo_root)
...
def get_changed_files(repo_root, base):
    ...
    try:
        base_check = subprocess.run(["git", "rev-parse", "--verify", f"{base}^{{commit}}"], capture_output=True, text=True, cwd=repo_root, env=env)
        if base_check.returncode != 0:
            return None
    except Exception:
        return None
    for cmd in [["git", "diff", "--name-only", f"{base}...HEAD"], ["git", "diff", "--name-only", base]]:
        try:
...
def main():
    ...
    report("INFO", f"Changed files: {len(changed_files)}, Allowed: {len(allowed)}, Forbidden: {len(forbidden)}, Out-of-scope: {len(out_of_scope)}")
    if allowed:
