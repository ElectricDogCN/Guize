...
class TestRepositoryBoundary(unittest.TestCase):
    ...
    def test_makefile_no_parent_directory_tests(self):
        ...
        for pattern in [r"\.\./tests/", r"/workspace/tests/"]:
            ...
            self.assertEqual(len(matches), 0, f"Makefile contains prohibited pattern '{pattern}': {matches}")
    ...
    def test_ci_no_parent_directory_tests(self):
        ...
        with open(ci_path, "r", encoding="utf-8") as handle:
            content = handle.read()
        lines = content.split("\n")
        code_lines = [line for line in lines if not line.strip().startswith("#") and "evidence/" not in line and "grep" not in line]
        code_content = "\n".join(code_lines)
        for pattern in [r"\.\./tests/", r"/workspace/tests/"]:
            matches = re.findall(pattern, code_content)
            self.assertEqual(len(matches), 0, f"CI workflow contains prohibited pattern '{pattern}': {matches}")
    ...
    def test_tests_directory_exists(self):
        ...
        test_files = [name for name in os.listdir(tests_dir) if name.startswith("test_") and name.endswith(".py")]
    ...
    def test_fixtures_locate_scripts_correctly(self):
        ...
        expected = os.path.join(REPO_ROOT, "scripts")
        self.assertEqual(fixtures.SCRIPTS_DIR, expected)
...
class TestTraeSpecsDirectory(unittest.TestCase):
    ...
    def test_specs_tasks_is_authoritative(self):
        ...
        self.assertTrue(os.path.exists(os.path.join(specs_tasks_dir, "GZ-001-repository-baseline.md")))
...
class TestCIWorkflowStatic(unittest.TestCase):
    ...
    def test_push_validates_all_main_updates(self):
        ...
        self.assertIn("main", push.get("branches", []), "Push validation must include main")
        self.assertNotIn("paths", push, "Main validation must not leave unvalidated path gaps")
        self.assertNotIn("paths-ignore", push, "Main validation must not ignore paths")
    ...
    def test_no_swallow_errors_with_or_true(self):
        ...
        with open(self.ci_path, "r", encoding="utf-8") as handle:
        ...
        for section in critical_sections:
            ...
            block = lines[index + 1 : index + 18]
            self.assertIn("run: |", [line.strip() for line in block], f"{section} must have run block")
            self.assertTrue(any("set -euo pipefail" in line for line in block), f"{section} must use strict shell")
            self.assertFalse(any("|| true" in line for line in block), f"{section} must not swallow errors")

    def test_no_auto_push_or_merge(self):
        with open(self.ci_path, "r", encoding="utf-8") as handle:
            content = handle.read().lower()
        for term in ["git push", "gh pr merge", "gh pr create", "auto-merge", "deploy to", "ssh "]:
            self.assertNotIn(term, content, f"Workflow must not contain '{term}'")

    def test_actions_use_explicit_versions(self):
        with open(self.ci_path, "r", encoding="utf-8") as handle:
            content = handle.read()
        for use in re.findall(r"^\s*uses:\s*(.+)$", content, re.MULTILINE):
            use = use.strip()
            self.assertIn("@", use)
            version = use.split("@", 1)[1]
            self.assertNotIn(version, {"main", "master", "latest"})
            self.assertTrue(version.startswith("v") or re.match(r"^[0-9a-f]{40}$", version))
