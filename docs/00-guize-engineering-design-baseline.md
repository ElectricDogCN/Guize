...
> 整理日期：2026-08-29
> 任务编号：GZ-002
> 来源基线：Guize V1 冻结方案、已批准需求、ADR-0001～0013、`docs/00～23`、`docs/appendices`、机器契约与部署 Profile
...
研发开工时优先阅读本文；原 `docs/00～23` 与 `docs/appendices/**` 继续保留，作为专题设计来源与历史背景。本文不替代已批准需求、机器契约、ADR 或 `AGENTS.md`。
...
本节严格继承 `AGENTS.md` 第 3 节，不得由任务文档或本总基线自行改变：

```text
1. 已批准的需求规格
→ 2. 已批准的 API、事件、数据 Schema 契约
→ 3. 已批准的 ADR
→ 4. 系统和模块设计文档（本研发设计总基线属于此层）
→ 5. AGENTS.md
...
如果代码、Task Spec、本总基线或历史文档与更高优先级来源冲突，不得自行选择低优先级结论。必须记录冲突，并按任务权限修复或建立新的需求/契约/ADR 决策。
...
**V1 不设置对外 Beta。** 所有纳入 V1 范围的能力必须达到已批准需求定义的生产级门禁后方可发布；POC、规划基线或局部实现完成不能被表述为生产就绪。
...
模块依赖原则：跨模块只调用公开 Application Service/Facade；禁止跨模块访问 Repository 或依赖 JPA Entity；跨模块写操作通过命令接口或领域事件；只读聚合通过显式 Query API；Shared/Common 不承载业务规则。
...
| `领域V1-AST-0006` | `DerivedArtifact` | ASR/OCR/摘要/Embedding 等结构化衍生物 | AI 原始输出与人工修订分版本保存 |
...
```text
iam / source / asset / storage / media / ai / search / task / policy / config / audit / outbox
```
...
- IAM：`iam_user`、`iam_role`、`iam_user_role`、`iam_group`、`iam_group_member`、`iam_asset_acl`、`iam_passkey_credential`、`iam_session`、`iam_login_event`。
- Source：`source_data_source`、`source_capability`、`source_sync_policy`、`source_sync_job`、`source_sync_checkpoint`、`source_object`、`source_object_history`、`source_credential_reference`。
- Asset：`asset_asset`、`asset_alias`、`asset_version`、`asset_content_hash`、`asset_duplicate_group`、`asset_merge_decision`、`asset_tag`、`asset_tag_assignment`。
- Media/Storage：`media_rendition`、`media_track`、`media_profile`、`media_quality_result`、`storage_backend`、`storage_replica`、`storage_cache_entry`、`storage_retention_policy`、`storage_retention_hold`、`storage_migration`、`storage_watermark_event`。
- AI/Search：`ai_provider`、`ai_model`、`ai_prompt`、`ai_pipeline`、`ai_derived_artifact`、`ai_quality_evaluation`、`search_index_record`、`search_embedding_record`、`search_rebuild_job`。
- Task/Policy/Config：`task_task`、`task_execution`、`task_worker`、`task_worker_capability`、`task_lease`、`policy_definition`、`policy_version`、`policy_test_case`、`policy_deployment`、`config_definition`、`config_version`、`config_approval`。
- Audit/Outbox：`audit_event`、`audit_security_event`、`outbox_event`、`outbox_consumer_offset`。
...
Outbox 与业务写入同一 PostgreSQL 事务；Redis 锁只用于性能/短期协调；关键聚合使用乐观锁；大文件正文和二进制产物不进入业务关系表正文列。
...
| 接口ID | Method/Path | 用途 |
|---|---|---|
| `接口V1-0101` | `POST /api/v1/data-sources` | 创建数据源 |
| `接口V1-0102` | `POST /api/v1/data-sources/{id}:probe` | 探测来源能力 |
| `接口V1-0103` | `POST /api/v1/data-sources/{id}:sync` | 创建同步任务 |
| `接口V1-0201` | `GET /api/v1/assets/{assetId}` | 查询资产 |
| `接口V1-0202` | `GET /api/v1/assets/{assetId}/versions` | 查询版本 |
| `接口V1-0203` | `POST /api/v1/assets/{assetId}:merge` | 确认合并 |
| `接口V1-0301` | `POST /api/v1/assets/{assetId}/playback-plan` | 生成播放计划 |
| `接口V1-0302` | `POST /api/v1/assets/{assetId}/transcode` | 创建转码任务 |
| `接口V1-0401` | `POST /api/v1/assets/{assetId}/enrich` | 创建 AI 加工任务 |
| `接口V1-0501` | `GET /api/v1/search` | 统一搜索 |
| `接口V1-0601` | `GET /api/v1/tasks/{taskId}` | 任务详情 |
| `接口V1-0602` | `POST /api/v1/tasks/{taskId}:pause` | 暂停任务 |
| `接口V1-0603` | `POST /api/v1/tasks/{taskId}:resume` | 恢复任务 |
| `接口V1-0604` | `POST /api/v1/tasks/{taskId}:cancel` | 取消任务 |
| `接口V1-0701` | `POST /api/v1/workers/register` | Worker 注册 |
| `接口V1-0702` | `POST /api/v1/workers/heartbeat` | Worker 心跳 |
| `接口V1-0801` | `POST /api/v1/policies/{id}:simulate` | 策略模拟 |
| `接口V1-0802` | `POST /api/v1/policies/{id}:publish` | 策略发布 |
...
核心事件包括 `source.object.observed`、`source.object.deleted`、`asset.created`、`asset.version.created`、`cache.complete.verified`、`replica.verified`、`rendition.published`、`artifact.published`、`task.status.changed`、`policy.published`。消费者按 `eventId` 幂等处理，不假设 exactly-once。
...
SSE 用于 AI 流式结果、ASR 分段、任务事件和配置助手，支持 `Last-Event-ID`。WebSocket 用于任务状态、Worker 在线状态、告警和临时转码会话；必须鉴权、心跳、限流和断线恢复。

### 7.6 Connector Plugin 契约

核心能力：`PROBE`、`AUTHENTICATE`、`LIST`、`STAT`、`RANGE_READ`、`FULL_READ`、`DOWNLOAD_URL`、`CHANGE_CURSOR`、`CHANGE_LIST`、`EVENT_SUBSCRIPTION`、`PROVIDER_HASH`、`VERSION`。

Connector 不得直连核心数据库、自行决定 Asset 身份、保存 Secret 明文、无限重试或绕过数据源预算和权限。

### 7.7 Worker 契约

Worker 注册上报 `workerId`、hardware、GPU/codecs/models、disk/memory、capabilities、softwareVersions、workingHours、dataClassification、budget。控制面返回短期凭据和允许执行能力；Worker 离线后 Lease 到期可重新调度，但不能把未知执行状态标记成功。

## 8. LiteFlow、Temporal 与 Task

### 8.1 LiteFlow 职责

LiteFlow 用于同步、快速、可解释的决策：缓存/TTL/优先级、转码 Profile、长期保留、AI 路由、商业 API、数据源扫描频率、流量预算和公开策略。
...
Temporal 用于下载/冷恢复、哈希/安全扫描、AV1/ABR/临时 H.264、ASR/OCR/VLM/翻译、Embedding/索引、正式副本迁移、备份/恢复/对账等长时间可恢复执行。
...
```text
Complete Cache:
REMOTE_ONLY → DOWNLOAD_QUEUED → DOWNLOADING → VALIDATING → COMPLETE_CACHE → PINNED_CACHE/PROMOTING → FORMAL_REPLICA → EVICTING → EVICTED

Replica:
PLANNED → COPYING → UPLOADED_UNVERIFIED → VERIFYING → VERIFIED → DEGRADED/UNAVAILABLE/CORRUPTED → REPAIRING → VERIFIED → DELETING → DELETED

Policy:
DRAFT → VALIDATING → TESTED → APPROVED → CANARY → PUBLISHED → DEPRECATED/ROLLED_BACK

Search:
PENDING → INDEXING → INDEXED → STALE/FAILED → REBUILD_REQUIRED
```

只有 `VERIFIED` Replica 可以计入正式恢复副本数量。
...
不承诺 exactly-once；通过 at-least-once + 幂等实现业务正确性。周期对账必须覆盖 SourceObject、CacheEntry、Replica、Task/Temporal、Outbox/Consumer、PostgreSQL/OpenSearch/Milvus、Backup 和 CredentialReference/OpenBao。未知状态优先标记和告警，不自动删除无法证明安全的对象。
...
`NORMAL → WARNING → CRITICAL → SAFETY_FLOOR`。绝对安全水位 500GB，不允许 AI 或普通策略突破。

### 10.4 Deployment Profile

`single-node-demo`、`control-plane`、`edge-media-a380`、`ai-worker-nvidia`、`av1-worker-ada`、`cpu-worker`、`search-node`、`observability-node`、`single-site-full`、`distributed-full`、`custom`。`guizectl` 与 Console 共用 Deployment Profile Schema。
...
public interface IdentityAccessFacade {
    AuthorizationDecision authorize(SubjectRef subject, ResourceRef resource, Action action, RequestContext context);
    SignedAccessGrant issueMediaGrant(SubjectRef subject, AssetVersionRef version, RenditionRef rendition, Duration ttl);
}
```

### 11.2 SourceManagementFacade / SourceConnector
...
归一原则：稳定 providerObjectId 优先；fingerprint 变化创建新 AssetVersion；完整 hash 相同形成高置信重复/移动候选；sample hash + size 只形成中置信候选；移动/改名不等于内容版本变化，来源删除不等于逻辑资产删除。

### 11.4 StorageLifecycleFacade
...
完整缓存：创建 Task/CacheEntry → 预留空间 → Range/断点下载 → Hash → 安全扫描 → 原子移动 → COMPLETE_CACHE → Outbox。

### 11.5 MediaControlFacade
...
播放选择：直接支持已有 Rendition → AV1 → 已有 HLS/DASH ABR → A380 临时 H.264 → 其他 Worker → 明确失败与修复建议。

### 11.6 TaskWorkflowFacade / RulePolicyFacade
...
### 11.8 SearchControlFacade / Configuration Center
...
统一搜索：Query Understanding → ACL Scope → PostgreSQL/OpenSearch Keyword → Milvus Vector → Fusion/RRF → Reranker → PostgreSQL Hydrate → Final ACL Check。

配置对象统一生命周期：Schema → Draft → Validate → Simulate → Approval → Canary → Publish → Observe → Confirm/Rollback → Audit。
...
审计至少记录 actor、source、action、resource、reason、before/after summary、result、traceId、taskId、approval、occurredAt。关联键统一使用 `traceId`、`requestId`、`taskId`、`workflowId`、`assetId`、`versionId`、`workerId`。

指标域覆盖 API、数据源、ATS、存储、播放、媒体、AI、任务、搜索、备份、安全。告警级别：Info / Warning / High / Critical / Emergency；高等级告警必须有 Runbook 和确认闭环。
...
- E2E：数据源→Asset→权限→Range 播放；数据源→完整缓存→AI→Search；删除→保护→恢复；Policy→Workflow→Replica；权限→Search→Playback；部署→升级→回滚。
...
`WP-M1-01` IAM / User / Role / ACL；`WP-M1-02` Asset / SourceObject / AssetVersion；`WP-M1-03` Connector SDK + WebDAV；`WP-M1-04` Incremental Sync / Checkpoint；`WP-M1-05` Playback Authorization；`WP-M1-06` ATS Range/Slice Integration；`WP-M1-07` Guize Player；`WP-M1-08` Task/Audit。

M1 闭环：WebDAV 接入 → 浏览 → Asset 归一 → 鉴权 → Range 播放 → 播放进度 → 监控/审计。

### M2 缓存与生命周期

`WP-M2-01` Complete Cache；`WP-M2-02` Hash/Security Scan；`WP-M2-03` Replica/Promotion；`WP-M2-04` Watermark/Eviction；`WP-M2-05` Retention/Hold/Delete Protection；`WP-M2-06` Backup/Restore/Traffic Budget；`WP-M2-07` Local/S3/HTTP Connector。

### M3 Media 与 AI

`WP-M3-01` Media Service + A380；`WP-M3-02` AV1 Master/ABR；`WP-M3-03` Temporary H.264；`WP-M3-04` ASR/Alignment/Diarization；`WP-M3-05` OCR/Keyframe/VLM；`WP-M3-06` Translation/Summary/Tag；`WP-M3-07` Thumbnail/Artifact Quality。

### M4 Search 与推荐

`WP-M4-01` PostgreSQL FTS；`WP-M4-02` OpenSearch；`WP-M4-03` Milvus/Embedding；`WP-M4-04` Fusion/Reranker；`WP-M4-05` Permission-safe Search；`WP-M4-06` Recommendation/Evaluation/Rebuild。

### M5 控制面与生产治理

`WP-M5-01` Configuration Center；`WP-M5-02` LiteFlow Policy UI/Runtime；`WP-M5-03` Temporal Ops/Worker Management；`WP-M5-04` `guizectl`/Bundle/Ansible；`WP-M5-05` Observability/Alert/Runbook；`WP-M5-06` GitOps/SWR/SBOM/Cosign；`WP-M5-07` Secrets/Backup/DR；`WP-M5-08` 剩余 Connector。

### M6 RC / Production

全量 E2E、性能、故障注入、安全、恢复、供应链、文档、运行观察与人工发布签署。M6 通过前不得对外宣称 V1 生产发布。

## 15. 研发任务拆分规则

每个工作包必须进一步拆成单独 Task，至少包含 Task ID、背景/目标/非目标、Allowed/Forbidden Scope、Requirement/Design/ADR 链接、OpenAPI/Event/DDL 契约、Class/Interface/DTO 规划、状态机/幂等/错误处理、Given/When/Then 验收、必须执行的测试、Evidence Contract 和回滚。
...
本文件不能替代以下后续任务：完整 `openapi.json`；各 Event Payload Schema；Flyway DDL/索引/约束/恢复说明；Java 包/类级设计；Temporal Workflow/Activity Contract；LiteFlow Node/Chain/EL Contract；Plugin/Worker Contract；前端页面与 API 映射。这些内容应按里程碑分批冻结，而不是在没有实现上下文时一次性虚构所有细节。
...
Issue
→ Task Spec
→ Requirement / Machine Contract / ADR / Design（按 AGENTS.md 权威顺序解析）
→ 独立 Branch
→ Implement
...
禁止直接修改 `main`、无 Task/Scope 的代码变更、无 Evidence 声称测试通过、无 ADR 改变长期架构决策、无 Migration 改数据库行为、无 OpenAPI/Event Schema 改机器契约、无回滚/恢复路径发布高风险变更，以及把 Agent 生成说明当成执行证据。
...
本文件从 GZ-002 开始作为研发设计主入口；原专题文档继续保留。任何实际实现状态仍以更高优先级的已批准需求/机器契约/ADR，以及 Git 仓库中的真实代码、测试、Evidence 和合并记录为准。